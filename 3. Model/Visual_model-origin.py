...
def plot_Test_Pred(x_tr, y_tr, x, y, r2_1, rmse_1, r2_2, rmse_2, num, unit, title, major, x_name):
    ...
    plt.xlabel('Experimental ' + x_name, fontsize='20', labelpad=8)
    plt.ylabel('Predicted ' + x_name, fontsize='20', labelpad=8)
    x_major_locator = MultipleLocator(major)
    y_major_locator = MultipleLocator(major)
    ax.xaxis.set_major_locator(x_major_locator)
    ax.yaxis.set_major_locator(y_major_locator)
    _ = plt.plot([-1000000, 6000000000], [-1000000, 6000000000], color='grey')
    plt.xscale('log')
    plt.yscale('log')
    plt.axis('square')
    plt.axis('equal')
    ...
    _ = plt.plot([-100, 6000], [-100, 6000], color='grey')
...
name_title = ['K', 'Qm']
name_title_plt = ['K', 'Q$\mathregular{_{m}}$']
...
for i in range(6):
    ...
    if j == 0:
        if k == 0:
            model = joblib.load('..\\3. Model\\save\\model-K-RF.pkl')
        elif k == 1:
            model = joblib.load('..\\3. Model\\save\\model-Qm-RF.pkl')
        ...
        if k == 0:
            model = joblib.load('..\\3. Model\\save\\model-K-Ada.pkl')
        elif k == 1:
            model = joblib.load('..\\3. Model\\save\\model-Qm-Ada.pkl')
        y_tr_pred_scaled = model.predict(x_train_scaled)
        y_tr_pred = y_scaler.inverse_transform(y_tr_pred_scaled.reshape(-1, 1)).flatten()
        y_pred_scaled = model.predict(x_test_scaled)
        y_pred = y_scaler.inverse_transform(y_pred_scaled.reshape(-1, 1)).flatten()
        ...
        if k == 0:
            model = joblib.load('..\\3. Model\\save\\model-K-ANN.pkl')
        elif k == 1:
            model = joblib.load('..\\3. Model\\save\\model-Qm-ANN.pkl')
        y_tr_pred_scaled = model.predict(x_train_scaled)
        y_tr_pred = y_scaler.inverse_transform(y_tr_pred_scaled.reshape(-1, 1)).flatten()
        y_pred_scaled = model.predict(x_test_scaled)
        y_pred = y_scaler.inverse_transform(y_pred_scaled.reshape(-1, 1)).flatten()
...
plt.savefig('..\\5. Plot\\2. Model\\model-plot-origin.jpg', dpi=300)
plt.show()
